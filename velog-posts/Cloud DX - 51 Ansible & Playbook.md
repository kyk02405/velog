...
<li>설명<ul>
<li><code>---</code><ul>
<li><code>플레이북</code>의 처음은 <code>---</code>으로 시작하여 <code>yml</code>파일임을 명시한다.</li>
<li><code>Shell Scripting</code>에서의 맨 첫줄에 <code>#!/bin/bash</code> 등과 유사하다.</li>
<li>일반적으로 말하는 <code>컴퓨터 언어(C/C++, Java, ...)</code>에서의 <code>Header File 선언(#include &lt;stdio.h&gt;</code>과도 유사하다.</li>
</ul>
</li>
<li><code>-name: Install nginx on linux</code><ul>
<li>현재 어떤 노드들에 대해서 <code>nginx 웹 서버</code>를 설치할 것인지를 지정한다.</li>
<li>주석의 의미를 띄고있다.</li>
</ul>
</li>
<li><code>gather(수집)_facts(사실): no</code><ul>
<li><code>플레이북</code> 실행 전에 호스트(작업할 대상)의 시스템 정보를 자동으로 수집한다.</li>
<li>여기서 말하는 <code>시스템 정보</code>는 <code>OS, CPU, RAM, NIC,...</code>을 말하낟.</li>
</ul>
</li>
<li><code>tasks:</code><ul>
<li><code>플레이북</code>이 실제로 수행되는 영역이다.</li>
</ul>
</li>
<li><code>- name:</code><ul>
<li>하단에서 실행될 명령들에 대한 명시적인 설명을 말한다. 즉, 주석의 의미이다.</li>
<li><code>name: install epel-release / yum: name=epel-release state=latest</code></li>
<li><code>name: install nginx web server / yum: name=nginx state=present</code></li>
<li><code>name: upload default index.html for web server / get_url: url=https://www.nginx.com dest=/usr/share/nginx/html/ mode=0644</code></li>
<li><code>name: start nginx web server / service: name=nginx state=started</code></li>
</ul>
</li>
<li><code>4개의 작업</code>을 <code>CLI Mode</code>로 실행한다고 하면 다음과 같이 하면 된다.<ul>
<li><code>ansible all -m yum -a &quot;name=epel-release state=latest&quot;</code></li>
<li><code>ansible all -m yum -a &quot;name=nginx state=present&quot;</code></li>
<li>(생략) <code>ansible all -m get_url -a &quot;url=https://www.nginx.com dest=/usr/share/nginx/html/ mode=0644&quot;</code></li>
<li><code>ansible all -m service -a &quot;name=nginx state=started&quot;</code>   (오류)</li>
<li><code>ansible all -m shell -a &quot;systemctl restart nginx&quot;</code>   (정상)</li>
</ul>
</li>
<li>주의할 점<ul>
<li><code>야믈(Yaml)</code>의 각 줄은 고유의 목적을 가지고 있으며 들여쓰기의 간격으로 해당 줄이 문법에 맞게 작성했는지를 판별한다.</li>
<li>(매우 중요) 따라서 에러가 없이 동작하게 하려면 들여쓰기는 매우 중요하다.</li>
<li>(매우 중요) 들여쓰기는 <code>탭</code>이 아닌 <code>스페이스</code>를 이용해서 작성해야 한다.</li>
</ul>
</li>
</ul>
</li>
